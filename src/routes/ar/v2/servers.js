...
var DeServers = {
    ...
    'deServers': function () {
	   ...
	   servers.push({'name': "mixdrop", 'shorten': "MP", 'v': 18, 'optional': true});
        ...
        servers.push({'name': "mediafire", 'shorten': "MF", 'headers': jawcloud_header, 'v': 19, 'optional': true});
...
const servers = async (req, res) => {
    ...
    let data_ = await axios.post("https://www.fembed.com/api/source/yky6ybed586m26k")
    let urls = Decoder.decode("https://www.fembed.com/api/source/yky6ybed586m26k", JSON.stringify(data_.data))
    res.status(200).json(JSON.parse(urls))
}
